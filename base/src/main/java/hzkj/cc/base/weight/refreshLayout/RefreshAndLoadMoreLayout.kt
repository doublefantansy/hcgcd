...
class RefreshAndLoadMoreLayout(context: Context, attributeSet: AttributeSet) :
                            ...
                            Handler().postDelayed({
                                if (showToast) {
                                    ...
                                    if (toastView.count != 0) {
                                        ...
                                        stateView.text = StateView.NODATA
                                        toast(
    ...
    fun showNetErrorState() {
        stateView.text = StateView.NETERROR
        stateView.visibility = View.VISIBLE
    }
...
//            stateView.text = StateView.NODATA
        stateView.layout(0, 0, width, height)
//        }
    }
    ...
    override fun onTouchEvent(e: MotionEvent?): Boolean {
        when (e?.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if ((recyclerView.layoutManager as LinearLayoutManager).findLastCompletelyVisibleItemPosition() == recyclerView.adapter!!.itemCount - 1 && canLoading && moveY < downY && !isRefresh && !isLoading && recyclerView.adapter!!.itemCount != 0) {
